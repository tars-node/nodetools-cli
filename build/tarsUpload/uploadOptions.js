...
class UplaodOptions {
    constructor() {
        ...
        this._application = "";
        this._server = "";
        this._option = tarsOptions_1.tokenOption;
    }
    async getOptions() {
        await this._checkPkg();
        //若本地缓存没有tars平台地址，则从终端读入
        if (!this._tarsurl) {
            let options = new tarsOptions_1.BaseOption();
            options.addChild(tarsOptions_1.tarsurlOption);
            options.addChild(tarsOptions_1.tokenOption);
